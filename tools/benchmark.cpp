...
int main(int argc,char **argv){
    if(argc!=9){
        std::fprintf(stderr,"usage: %s MODEL BACKEND_MODULE CPU|Vulkan DEVICE THREADS FIXTURE FRAMES ITERATIONS\n",argv[0]);
        return 2;
    }
    try{
        ...
        status=gemx_infer(session.get(),&input,motion.data(),motion.size(),camera.data(),camera.size(),error,sizeof(error));
        if(status!=GEMX_OK)throw std::runtime_error(error);
        ...
        std::printf("mode=%s device=%s frames=%u threads=%u iterations=%u wall_ms=%.3f throughput_fps=%.2f "
                    ...
                    "offline",gemx_session_device(session.get()),frames,threads,iterations,wall_ms,
                    frames*1000./wall_ms,
